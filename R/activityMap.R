#' Use filtered data to draw the activity map.
#'
...
#'
#' @param filtered_position data filtered from positionFilter() function
...
#' @export
#' @examples
#' activityMap(filtered_position, colour = "#00FFFF", opacity = 1, weight = 1, fillOpacity = 0.2)
#' activity_map <- activity_map(filtered_position, colour = "red")
#' activity_map
activityMap <- function(filtered_position,
                         colour = "#00FFFF",
                         opacity = 1,
                         weight = 1,
                         fillOpacity = 0.2){
    num_data <- length(filtered_position)
    ...
    for(i in 1:num_data) {
        activity_map <- activity_map %>%
            addPolylines(
                data = as.matrix(filtered_position[[i]]),
                color = colour,
                opacity = opacity,
                weight = weight,
